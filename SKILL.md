...
- 反馈类：`反馈`、`能不能优化`、`能不能迭代`、`希望加一个这个功能`、`问题`、`阻塞`、`故障`、`异常`、`升级`、`投诉`、`延迟`
- 业务类：`上线`、`发布`、`变更`、`迭代`、`优化`
- 会议类：`周会`、`复盘`、`纪要`、`议题`、`同步会`

关键词优先级：业务类和反馈类优先；当同一批候选消息数量较多时，优先保留命中业务类或反馈类关键词的消息，再补充决策类、行动类、会议类消息。
...
- 每个群的输出应优先覆盖不同主题：业务进展、反馈/问题、公告/决策、行动项、群主/管理员表态、高互动讨论；业务类和反馈类关键词命中的消息优先级高于其他关键词命中
